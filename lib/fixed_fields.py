...
def iterate_fixed_fields(file_path, fields, RECORD_TYPE=None, full_only=True):
    ...
    log.debug('ingesting %s' % (file_name))
    if not fields or not sum(fields.values(), []):
        log.warning('No fields defined for %s' % (file_name))
        return
    with open(file_path, 'r') as f:
        ...
        for fi, line in enumerate(f.readlines()):
            ...
            if sum(field_lens) != len(line)-offset:
                raise Exception('%s Line "%s" (len %d) doesn\'t match spec (len %d): %s %r' % (
                    file_name, line, len(line), sum(field_lens), record_type,
                    fields[record_type]))
            ...
            if fi % 10 == 0:
                ...
                if last_rtime != rtime:
                    ...
                    log.debug('%s %s lines per second' % (file_name, per_sec))
                    last_fi = fi
                    stime = t_time()
                    last_rtime = rtime
